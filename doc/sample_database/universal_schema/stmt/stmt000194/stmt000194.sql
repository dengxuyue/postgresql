SELECT	"State"	,"IncomePerHousehold" AS "_twm_Valu"	,"_twm_Sad" AS "_twm_Sad"	,CAST(SUM(1) OVER (PARTITION BY "State" 
ORDER	BY "IncomePerHousehold" ROWS UNBOUNDED PRECEDING) AS FLOAT) AS "_twm_Rnk"	FROM "universal_schema"."demographics" "T1",
