...
from	( 
    select	ast.store_class store_class
            --, ast.average_feet average_feet
            ,ast.associate associate, sum(st.MKB_Rev_Amt) as rev 
    from	( 
        select	inner_store.store_class store_class, -- inner_store.average_feet average_feet,
                Associate_Party_Id Associate 
        from	( 
            select	l.Location_Id location_id, 
                    l.Location_Name as name,
                    case	
                        when l.Location_Total_Area_Meas < 50000 then '40-50K Feet' 
...
--                 ,avg(Location_Total_Area_Meas) over (partition by store_class) as average_feet 
            from	universal_schema.location l 
            group by location_id
            ) inner_store, 
        universal_schema.associate a 
        where	inner_store.location_id = a.location_id 
                and	Position_Id not in (919,914,915,918) 
        ) ast , 
