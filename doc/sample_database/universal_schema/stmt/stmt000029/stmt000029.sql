...
CASE	WHEN A.REV >=4000 THEN 'PREMIER'
	WHEN	A.REV >=3000 
	AND	A.REV <4000 THEN 'GOLD'
	WHEN	A.REV >=2000 
	AND	A.REV < 3000 THEN 'SILVER'
	WHEN	A.REV >= 0 
	AND	A.REV < 2000 THEN 'BRONZE'
END	AS ClUB_LEVEL,
SUM(B.REFUNDED)/18 "AVE OF 18 MONTHS REFUNDED",
SUM(C.REFUNDED) "30 DAYS REFUNDED",
SUM(C.REFUNDED) * 100 / (SUM(B.REFUNDED) / 18) "%"
FROM	
(
...
GROUP	BY S.REWARD_CD
)C(REWARD_CD,REFUNDED)
WHERE	
A.REWARD_CD = B.REWARD_CD
	AND	C.REWARD_CD = B.REWARD_CD
GROUP	BY CLUB_LEVEL